...
func TestCtyToAny(t *testing.T) {
	...
	t.Run("capsule types", func(t *testing.T) {
		// Create a simple test capsule with a string value
		testValue := "test capsule content"
		capsuleType := cty.Capsule("test", reflect.TypeOf(testValue))
		capsule := cty.CapsuleVal(capsuleType, &testValue)

		result, err := CtyToAny(capsule)
		require.NoError(t, err)

		// Should unwrap to the encapsulated value
		stringPtr, ok := result.(*string)
		require.True(t, ok, "Expected *string")
		assert.NotNil(t, stringPtr)
		assert.Equal(t, testValue, *stringPtr)
	})

	t.Run("nested with capsules", func(t *testing.T) {
		// Create an object containing a capsule
		testValue := "nested capsule test"
		capsuleType := cty.Capsule("test", reflect.TypeOf(testValue))
		capsule := cty.CapsuleVal(capsuleType, &testValue)
		...
		stringPtr, ok := resultMap["capsule"].(*string)
		require.True(t, ok)
		assert.Equal(t, testValue, *stringPtr)
